...
slides = [
  ...
  {"mod":7,"title":"Pourquoi utiliser le Masque ?","sub":"","type":"theory","points":["Appliquer une **charte graphique** en une seule opération","Garantir la **cohérence visuelle** sur toutes les diapositives","Ajouter logo, couleurs, polices **une seule fois** pour toute la présentation","Éviter de modifier chaque diapositive manuellement","2 niveaux : **Masque de diapositive** (parent) et **Masques de disposition** (enfants)"]},
...
colors = {0:"#C0392B",1:"#E74C3C",2:"#E67E22",3:"#27AE60",4:"#2980B9",5:"#8E44AD",6:"#16A085",7:"#2C3E50",8:"#D35400"}
mod_names = {0:"",1:"Module 1",2:"Module 2",3:"Module 3",4:"Module 4",5:"Module 5",6:"Module 6",7:"Module 7",8:"Module 8"}

slide_html = ""
for i, s in enumerate(slides):
    c = colors.get(s["mod"], "#333")
    t = s["type"]
    mn = mod_names[s["mod"]]
    header = "" if t == "cover" else f'<div class="slide-header" style="background:{c}">{mn}</div>'
    if t == "cover":
        extra = s.get("extra", "")
        extra_html = f'<p class="cover-extra">{extra}</p>' if extra else ""
        body = f'<div class="cover-content"><div class="cover-icon">📊</div><h1>{s["title"]}</h1><p class="cover-sub">{s["sub"]}</p>{extra_html}</div>'
    elif t == "module":
        body = f'<div class="module-content" style="border-left:8px solid {c}"><span class="module-number" style="color:{c}">{mn}</span><h1>{s["title"]}</h1><p class="module-sub">{s["sub"]}</p></div>'
    elif t == "theory":
        pts = "".join([f"<li>{p}</li>" for p in s["points"]])
        body = f'<div class="slide-body"><h2 class="slide-title" style="color:{c}">{s["title"]}</h2><ul class="theory-list">{pts}</ul></div>'
    else:
        steps = "".join([f"<li>{st}</li>" for st in s["steps"]])
        body = f'<div class="slide-body"><div class="ex-header" style="background:{c}"><span>🖱️</span><h2>{s["title"]}</h2><span class="ex-dur">{s["sub"]}</span></div><ol class="step-list">{steps}</ol></div>'
    slide_html += f'<div class="slide" id="s{i}" style="display:none">{header}{body}</div>\n'
...
.cover-extra{{font-size:14px;opacity:.85;margin-top:24px;max-width:700px;line-height:1.7;background:rgba(255,255,255,.15);padding:20px;border-radius:10px;text-align:left}}
...
.step-list li strong{{color:#1a1a2e}}
.step-list li kbd{{background:#eee;border:1px solid #ccc;border-radius:3px;padding:1px 5px;font-size:12px;font-family:monospace;font-style:normal}}
.theory-list li strong{{color:#1a1a2e}}
...
.progress-bar{{height:100%;background:linear-gradient(90deg,#E74C3C,#8E44AD);transition:width .35s cubic-bezier(.4,0,.2,1)}}
</style>
</head>
<body>
...
let cur=0;
document.querySelectorAll('strong').forEach(e=>{{e.style.fontWeight='700'}});
function parse(el){{
  el.querySelectorAll('li,p,h2').forEach(n=>{{
...
with open("cours_powerpoint.html", "w", encoding="utf-8") as f:
    f.write(html)
