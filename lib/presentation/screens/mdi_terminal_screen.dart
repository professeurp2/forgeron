...
class _MDITerminalScreenState extends ConsumerState<MDITerminalScreen> {
  ...
  Widget build(BuildContext context) {
    final terminalPanel = Column(children: [
      ...
      // Input bar
      Container(key: TutorialKeys.mdiInput, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), decoration: const BoxDecoration(color: AppColors.surface, border: Border(top: BorderSide(color: AppColors.surfaceBorder))),
        child: Row(children: [
          ...
          Expanded(child: Container(height: 44, padding: const EdgeInsets.symmetric(horizontal: 16), decoration: BoxDecoration(color: AppColors.surfaceBright, borderRadius: BorderRadius.circular(4), border: Border.all(color: AppColors.surfaceBorder)),
            child: Align(alignment: Alignment.centerLeft, child: TextField(
              ...
              decoration: const InputDecoration(border: InputBorder.none, hintText: 'Saisir commande...', hintStyle: TextStyle(color: AppColors.textDisabled)),
              onSubmitted: _sendCommand,
            )))),
          const SizedBox(width: 12),
          IconButton(onPressed: () {}, icon: const Icon(Icons.history, color: AppColors.textSecondary)),
          const SizedBox(width: 8),
          ElevatedButton.icon(onPressed: () => _sendCommand(_controller.text), icon: const Icon(Icons.send, size: 14), label: const Text('ENVOYER', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900)), style: ElevatedButton.styleFrom(minimumSize: const Size(0, 48))),
        ])),
    ]);

    final sidePanel = SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
