...
/// Autorise ou non le **flux MJPEG continu**.
///
/// La caméra et le téléphone sont tous les deux clients du point d'accès porté
/// par l'ESP32 FluidNC : une seule radio 2,4 GHz pour tout le monde. Un flux
/// MJPEG y consomme en permanence la bande passante que le streaming G-code
/// utilise pour envoyer ses lignes et recevoir les rapports d'état.
///
/// Conséquence : **pas de flux continu pendant une passe**. Le panneau retombe
/// automatiquement sur des captures espacées, qui coûtent une poignée de kilo-
/// octets toutes les quelques secondes au lieu d'un débit soutenu.
...
/// On garde un frein pendant la coupe pour laisser la bande passante au
/// G-code, mais assez léger pour que l'image reste utile à la surveillance.
final cameraSnapshotIntervalProvider =
    Provider<Duration>((ref) => const Duration(milliseconds: 1200));
...
const kCameraIdleGap = Duration(milliseconds: 150);
