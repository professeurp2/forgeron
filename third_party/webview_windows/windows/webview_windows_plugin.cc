...
class WebviewWindowsPlugin : public flutter::Plugin {
 ...
 private:
  // FORGERON : partagés entre TOUTES les instances du plugin, pas un membre
  // par instance. Voir la note au-dessus de la définition (fin de fichier)
  // pour la raison.
...
// FORGERON : patch local (voir pubspec.yaml, dependency_overrides).
//
// `WebviewPlatform` crée un `DispatcherQueueController` WinRT dans son
// constructeur, réservé au thread courant — Windows n'en autorise qu'un par
// thread (`CreateDispatcherQueueController` échoue sinon). Avant ce patch,
// `platform_` et `webview_host_` étaient des membres PAR INSTANCE : chaque
// fenêtre ouverte par `desktop_multi_window` enregistre son propre moteur
// Flutter, donc sa propre instance de `WebviewWindowsPlugin` — mais toutes
// les fenêtres tournent sur le MÊME thread (`desktop_multi_window` ne crée
// aucun thread par fenêtre). La première visionneuse 3D réussissait son
// initialisation ; la seconde échouait avec « The platform is not
// supported » (`IsSupported()` renvoyant `valid_ = false`), venu du refus de
// `CreateDispatcherQueueController` pour un thread qui en possède déjà un.
//
// Rendre ces deux membres STATIQUES — partagés par toutes les instances du
// plugin dans le process — règle le problème à sa source : une seule
// plateforme, un seul environnement WebView2, correct puisqu'ils sont de
// toute façon déjà sur le même thread. Chaque fenêtre garde son propre
// `WebviewBridge` (la carte `instances_`, restée par instance) : la vue
// affichée reste bien la sienne, seul l'environnement sous-jacent est
// partagé — exactement ce que fait déjà ce plugin pour plusieurs webviews
// OUVERTES DANS UNE MÊME fenêtre ; on étend juste ce partage à tout le
// process.
